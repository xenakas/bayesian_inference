...
transformed parameters {
  vector[T] res;
  for (t in 1:T)
    res[t] = y[t] - (m1*d1[t] + m2*d2[t] + m3*d3[t]     + c1*d1[t]*trend[t] + c2*d2[t]*trend[t]  + c3*d3[t]*trend[t]   + f_mat[t,]*cfour  + ccdd * dcdd[t] + chdd * dhdd[t] +  csat * dsat[t] + csun * dsun[t] + cmon * dmon[t] + chol * dhol[t] );
}
model {
