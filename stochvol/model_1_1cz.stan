...
model {
  res[1] ~ normal(cgdp * gdp[1] + cgas * pgas[1] + ccoal * pcoal[1] + ctempp * dtp[1] * temp[1] + ctempn * dtn[1] * temp[1], sigma);
  for (t in 2:T)
    res[t] ~ normal(res[t-1] +  cgdp * gdp[t] + cgas * pgas[t] + ccoal * pcoal[t]  +  ctempp * dtp[t] * temp[t] + ctempn * dtn[t] * temp[t] + ctemppl * dtp[t-1] * temp[t-1] + ctempnl * dtn[t-1] * temp[t-1], sigma);
